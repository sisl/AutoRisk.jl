export
    OrigMultiFeatureExtractor,
    length,
    get_start_of_feature_section,
    pull_features!
    
type OrigMultiFeatureExtractor <: AbstractFeatureExtractor
...
function OrigMultiFeatureExtractor(
    extract_core::Bool,
...
AutomotiveDrivingModels.rec_length(ext::OrigMultiFeatureExtractor) = ext.extract_temporal ? 3 : 1
function Base.length(ext::OrigMultiFeatureExtractor)
...
function get_start_of_feature_section(ext::OrigMultiFeatureExtractor)
...
function AutomotiveDrivingModels.pull_features!{F<:AbstractFloat}(ext::OrigMultiFeatureExtractor,
    features::Vector{F},
    rec::SceneRecord,
    roadway::Roadway,
    vehicle_index::Int,
