...
type Dataset
    ...
    """
    function Dataset(filepath::String, feature_dim::Int64, target_dim::Int64,
...
function initialize!(dataset::Dataset)
    ...
    features = d_create(risk_dataset, "features", datatype(Float64), 
                dataspace(dataset.feature_dim, dataset.max_num_samples), 
                "chunk", (dataset.feature_dim, dataset.chunk_dim))
...
function finalize!(dataset::Dataset)
    ...
    set_dims!(dataset.features, (dataset.feature_dim, dataset.next_idx - 1))
...
function aggregate_datasets(input_filepaths::Vector{String}, 
    ...
    for (idx, filepath) in enumerate(input_filepaths)
        h5open(filepath, "r") do proc_file
            ...
            num_proc_features, num_proc_samples = size(proc_file["risk/features"])
    ...
    for filepath in input_filepaths
        ...
        num_proc_samples = size(proc_file["risk/features"], 2)
