...
function GaussianMLPDriver{A <: DriveAction}(::Type{A}, 
    ...
    GaussianMLPDriver{A, eltype(input_vec), eltype(output), typeof(extractor), typeof(mvnormal)}(net, rec, pass, input_vec, output, extractor, mvnormal, context, features)
end
...
function AutomotiveDrivingModels.observe!{A,F,G,E,P}(
    ...
    # println(o[:,vehicle_index])
    model.net[:hidden_0].input = o[:,vehicle_index]
    forward!(model.pass)
    # println(model.output)
    # readline()

    model.output[1] = max(min(model.output[1], 10.), -10.)
    model.output[2] = max(min(model.output[2], -1.), 1.)
    copy!(model.mvnormal.μ, model.output[1:2])
    return model
end
Base.rand{A,F,G,E,P}(model::GaussianMLPDriver{A,F,G,E,P}) = convert(
